...
class Model(object):
    ...
    def train_discriminator_wgan(self, discriminator, real, fake):
...
#             d_step = tf.group(*(tf.assign(var, tf.clip_by_value(var, -clip, clip)) for var in d_var))
        ...
        [v.assign(tf.clip_by_value(v, -0.01, 0.01)) for v in theta_d]
        
        return d_loss
    
    def train_discriminator_dcgan(self, discriminator, real_data, fake_data):
        # Real Data
        prediction_real = discriminator(real_data)
        error_real =  tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=prediction_real, labels=tf.ones_like(prediction_real)))
        
        # Fake Data
        prediction_fake = discriminator(fake_data)
        error_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=prediction_fake, labels=tf.zeros_like(prediction_fake))) 
    ...
    def define_graph(self):
        ...
        #discriminator_loss = self.train_discriminator_dcgan(discriminator, self.x, params)
        
        x_features, y_features = self.vgg19.get_VGG_features(self.x, params)
        discriminator_loss = self.train_discriminator_dcgan(discriminator, x_features[0], y_features[0])
        ...
        ### Added for CIS680 ###
        loss = likelihood_loss + kl_weight * kl_loss + discriminator_loss
        ### CIS680 code ends ###
...
#         UNCOMMENT THIS LINE FOR TRANSFER
#         initializer_op = tf.global_variables_initializer(self.variables)
# COMMENT THIS LINE FOR TRANSFER
    ...
    def restore_graph(self, restore_path):
        self.writer = tf.summary.FileWriter(
                ...
                session.graph)
        self.saver = tf.train.Saver(self.variables)
        self.saver = tf.train.import_meta_graph('./deepfashion1_pretrained/checkpoints/model.ckpt-100000.meta')
        self.saver.restore(session,  './deepfashion1_pretrained/checkpoints/model.ckpt-100000')
        #self.saver = tf.train.import_meta_graph('./log_models/DCGAN_100000_steps/checkpoints/model.ckpt-100000.meta')
        #self.saver.restore(session,  './log_models/DCGAN_100000_steps/checkpoints/model.ckpt-100000')
